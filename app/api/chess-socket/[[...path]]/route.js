...
export async function GET(request, { params }) {
  ...
  if (path.length === 0) {
    // Root path - return general info and all rooms
  ...
  if (path[0] === 'queue') {
    ...
    return NextResponse.json({
      ...
      waitingPlayers: matchmakingQueue.map(player => player.name)
    });
  }
...
export async function POST(request, { params }) {
  try {
    ...
    if (path.length === 0) {
      ...
      if (data.action === 'roomStatus' && data.roomId) {
...
// Configuration for API route - new syntax
export const dynamic = 'force-dynamic';
export const runtime = 'nodejs';

// This signal is needed for compatibility with Next.js new app router feature
