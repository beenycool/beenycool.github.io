...
export const safeConstructApiUrl = (endpoint, baseUrl = DEFAULT_BACKEND_URL) => {
  try {
    ...
    if (typeof constructApiUrl === 'function') {
      return constructApiUrl(endpoint);
    }
  } catch (error) {
    console.warn('Error using imported constructApiUrl, falling back to safe implementation');
  }
